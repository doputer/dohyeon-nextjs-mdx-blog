...
import type { Terrain } from '~/perlin-noise-terrain/perlin-noise-terrain';
...
interface Props {
  ...
  control: {
    ...
    togglePause: () => void;
    regenerate: () => void;
    setTerrainValue: (key: keyof Terrain, value: number) => void;
  };
}

const SLIDERS: { key: keyof Terrain; label: string; min: number; max: number; step: number }[] = [
  { key: 'octaves', label: '옥타브', min: 1, max: 6, step: 1 },
  { key: 'amplitude', label: '진폭', min: 0.5, max: 2, step: 0.1 },
];
  ...
  return (
    <div className="flex flex-col gap-2 rounded border border-line bg-surface p-3">
          ...
          return (
            <label key={key} className="flex items-center gap-2.5 text-xs text-muted select-none">
              ...
              <input
                ...
                step={step}
                value={value}
                onChange={(e) => control.setTerrainValue(key, Number(e.target.value))}
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted select-none">노이즈 스크롤</span>
