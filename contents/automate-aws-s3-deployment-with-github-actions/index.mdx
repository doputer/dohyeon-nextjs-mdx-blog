...
GitHub Actions는 GitHub에서 제공하는 CI/CD툴이다. 다시 말해서 Workflow를 자동화할 수 있게 도와준다.

GitHub Actions에 관해서는 다른 글에서 자세하게 다루도록 하고 이 글에서는 내가 AWS S3에 배포 자동화를 하는 과정과 그 과정에서 발생한 문제를 해결하는 것에 관한 글을 쓰려고 한다.

## 1. AWS IAM

**AWS IAM -> 액세스 관리 -> 사용자 -> 사용자 추가**

우선 AWS IAM 설정을 해줘야 한다.
...
뒤로 넘기다가 완료가 되면 **.csv 다운로드**를 선택해서 액세스 키를 다운로드한다.

> 비밀 키는 다시 다운로드할 수 없고 분실한 경우 새로 발급 받아야 한다!
