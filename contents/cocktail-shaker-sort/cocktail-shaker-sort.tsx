...
import Chart from '#/cocktail-shaker-sort/chart';
import Controller from '#/cocktail-shaker-sort/controller';
import { cocktailShakerSort, shuffle, sleep } from '#/cocktail-shaker-sort/engine';
...
const Run = ({ speed, speedRef, increaseSpeed, restart }: RunProps) => {
  ...
  const [sortedBefore, setSortedBefore] = useState(0);
  ...
  useEffect(() => {
    ...
    const animate = async () => {
      ...
      for (const step of cocktailShakerSort(seed)) {
        ...
        setSortedBefore(step.sortedBefore);
