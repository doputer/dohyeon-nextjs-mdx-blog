...
처음에는 특정 날짜부터 시작된 문제로 보고 그 시점을 찾는 데서 출발했는데, 이 짐작부터 빗나갔다. 더 오래된 구간까지 거슬러 올라가 봐도 그래프는 이미 같은 기울기로 오르고 있었기 때문이다.
...
D -->|아니오| E["다른 arena를 찾아 락 대기"]
D -->|예| F["mmap()으로 새 arena 생성"]
