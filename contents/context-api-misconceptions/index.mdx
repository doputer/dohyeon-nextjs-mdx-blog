...
하지만 `Context API`에 대해 학습하다보면 여러 오해와 잘못된 인식들을 접하게 됩니다.
...
다시 말해 `Context API`는 이미 존재하는 상태를 어디서든 접근 가능하게 만드는 것이고, **전역 상태 관리 라이브러리**는 상태 자체를 관리하고 최적화하는 것입니다.
...
그래서 `Baz` 컴포넌트의 버튼을 눌렀을 때 `Bar` 컴포넌트는 리렌더링 되지 않을거라 기대합니다.
...
그래서 `Baz` 컴포넌트에서 `value2` 상태를 바꿔도 `Bar` 컴포넌트에서는 리렌더링 되지 않을거라 기대합니다.

그러나 `React`는 `Context`가 변경되면 이를 구독하고 있는 모든 컴포넌트를 리렌더링 시킵니다.
...
const Provider = ({ children }) => {
  ...
  return <Context.Provider value={{ value1, setValue1, value2, setValue2 }}>{children}</Context.Provider>;
};
...
이는 `Provider`를 상태 별로 분리해서 해결할 수 있습니다.
