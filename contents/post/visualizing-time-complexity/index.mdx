...
시간 복잡도는 컴퓨터 프로그램의 입력 값과 연산 수행 시간의 상관관계를 나타내는 척도다. 주로 빅-오 표기법을 사용하는데 `O(n)` , `O(n^2)` , `O(n^3)` 등이 있다.
...
clock() 함수와 비교했을 때 clock()은 초당 1,000번의 측정을 통해 1ms의 시간을 측정할 수 있는 데 비해, QueryPerformanceCounter()는 초당 10,000,000번의 측정으로 0.1us의 시간까지 측정할 수 있다. 초당 클럭수는 `time.h`를 헤더로 추가한 후 `CLOCKS_PER_SEC`을 출력하여 알 수 있고, 타이머의 주파수는 QueryPerformanceFrequency()를 통해 알 수 있다.
...
이제 직접 코드로 구현해볼 차례다. 코드는 다음과 같이 작동한다.
