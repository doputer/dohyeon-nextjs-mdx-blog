...
이제 어느정도 구색을 갖춘 상태 저장소가 되었습니다.

여기까지 구현한 코드만으로도 전역 상태를 다루기에 충분하지만 조금 더 편리하게 쓰기 위해 개선해보겠습니다.
...
사실 상태를 관리하는 코어 로직은 Vanilla로 이미 작성되어 있어서 이 인터페이스를 노출해주기만 하면되는데요. 그렇다고 애써 없앤 `store` 불러오는 로직을 다시 만들고 싶지는 않습니다.
...
끝으로 지금까지는 `Hook`을 통해서만 상태를 가져오거나 변경했습니다. 하지만 같은 상태의 최신 값을 읽어야하거나 컴포넌트 내부에서 `Hook`이 들어갈 수 없는 위치에서도 상태를 변경해야하는 요구 사항이 있습니다.

그래서 상태 저장소를 생성하면서 내부에서 직접 상태를 읽고 쓸 수 있는 메서드를 제공해보겠습니다.
...
두렵게만 느껴졌던 라이브러리의 내부를 직접 들여다보니 생각보다 훨씬 단순하고 명확한 원리로 동작한다는 것을 알게되었습니다. 특별한 마법이 아니라 제가 알고 있던 개념들이 잘 조합되어서 만들어져 있었습니다.
